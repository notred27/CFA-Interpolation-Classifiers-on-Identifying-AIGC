function percentDiff = bicubic3D(img,src)
% bicubic3D: Return a 1x3 vector of RGB percent differences using bicubic interpolation
...
percentDiff = {double(sum(diff(:,:,1), "all")) / ((w-14) * (h-14)), double(sum(diff(:,:,2), "all")) / ((w-14) * (h-14)), double(sum(diff(:,:,3), "all")) / ((w-14) * (h-14))};
end
